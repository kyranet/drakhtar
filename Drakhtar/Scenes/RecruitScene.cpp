...
void RecruitScene::preload()
{
  ...
  if(PlayerData::getInstance()->getLevel() >= 2)
...
int RecruitScene::getCost(string type)
{
  return costs_[type];
}
...
void RecruitScene::addUnit(string textureName, int position)
{
  ...
  auto lessButton = new GameObject(this,
  ...
  lessButton->addChild(new Text(this,
    FontManager::getInstance()->get("Retron2000"),
    Vector2D<int>(WIN_WIDTH / 3.47 - WIN_WIDTH / 12.8,
      WIN_HEIGHT / 2 + WIN_HEIGHT / 6 * position),
    { 0, 0, 0, 255 },
    "-",
  ...
  auto moreButton = new GameObject(this,
    TextureManager::get("Quantity-Button"),
    Vector2D<int>(WIN_WIDTH/3.47 + WIN_WIDTH / 12.8,
      WIN_HEIGHT / 2 + WIN_HEIGHT / 6 * position),
    Vector2D<int>(WIN_WIDTH / 12.92 * 1.5,
  ...
  moreButton->addChild(new Text(this,
  ...
  auto text = new Text(this, FontManager::get("Retron2000"), 
