...
TextureInfo *TextureManager::add(string name, string path, ushort columns,
                                 ushort rows, SDL_RendererFlip flip) {
...
void TextureManager::init(SDL_Renderer *renderer) {
  while (!stack_.empty()) {
    ...
    auto texture =
        (new Texture(renderer))->loadFromImage(info->path, info->rows, info->columns);
...
Texture *TextureManager::get(string name) { return getInstance()->map_[name]; }
